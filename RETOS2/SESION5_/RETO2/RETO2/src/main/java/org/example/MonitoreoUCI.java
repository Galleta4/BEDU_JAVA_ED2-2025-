...
public class MonitoreoUCI {
    ...
    // signos vitales aleatorios por paciente
    private static Flux<EventoVital> generarSignosVitales(int pacienteId, Random random) {
        return Flux.interval(Duration.ofMillis(300))
                .map(tick -> {
                    ...
                    int pas = 80 + random.nextInt(80); // 80-160 mmHg
                    int pad = 50 + random.nextInt(50); // 50-100 mmHg
    ...
    static class EventoVital {
        ...
        public String toString() {
            ...
            if (pas < 90 || pas > 140 || pad < 60 || pad > 90)
                sb.append("Paciente").append(pacienteId).append("-PA crítica:").append(pas).append("/")
                        .append(pad).append(" mmHg\n");
